...
make_data_mcc_lfs_monthly <- function(data_mcc_lfs,
  ...
  data_mcc_lfs_monthly <- data_mcc_lfs %>%
    
    # Create unique month ID variable
    mutate(year_month = floor_date(date, "month")) %>%
    
    mutate(month_id = as.integer(time_length(interval(min(year_month), year_month), "month") + 1)) %>%
