...
get_hyperparam_loss_elastic_net <- function(par,
  ...
  loss_pseudo_controls <- map_dbl(n_controls, function(x){
    ...
    pseudo_Y1_pre <- Y0_pre[,x]
    pseudo_Y1_post <- Y0_post[,x]
    
    # Generate pseudo-control group for pre/post treatment
    pseudo_Y0_pre <- Y0_pre[,-x]
    pseudo_Y0_post <- Y0_post[,-x]
    
    # Get predictions on pseudo-post-treatment outcomes
    pseudo_Y1_post_hat <- predict_Y_post_elastic_net(Y1_pre = pseudo_Y1_pre,
                                                   Y1_post = pseudo_Y1_post,
                                                   Y0_pre = pseudo_Y0_pre,
                                                   Y0_post = pseudo_Y0_post,
                                                   alpha = alpha,
                                                   lambda = lambda)
    
    # Generate mean squared difference from true pseudo post-treatment outcomes
    mean_squared_pred_error <- mean((pseudo_Y1_post - pseudo_Y1_post_hat)^2)
